...
def version_history(src_files, bug_reports):
    ...
    for report in bug_reports.values():
        ...
        input_bug_report = datetime.datetime.strptime(report.opendate, '%Y-%m-%d %H:%M:%S').timestamp()
        for rep in bug_reports.values():
            commit_date = datetime.datetime.strptime(rep.fixdate, '%Y-%m-%d %H:%M:%S').timestamp()
